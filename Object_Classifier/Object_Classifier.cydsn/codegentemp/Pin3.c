...
#include "Pin3.h"
...
#if defined(Pin3__PC)
    #define Pin3_SetP4PinDriveMode(shift, mode)  \
    do { \
        Pin3_PC =   (Pin3_PC & \
                                (uint32)(~(uint32)(Pin3_DRIVE_MODE_IND_MASK << \
                                (Pin3_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Pin3_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Pin3_SetP4PinDriveMode(shift, mode)  \
        do { \
            Pin3_USBIO_CTRL_REG = (Pin3_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Pin3_DRIVE_MODE_IND_MASK << \
                                    (Pin3_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Pin3_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Pin3_SetP4PinDriveMode(Pin3__0__SHIFT, mode);
    }
#endif
...
void Pin3_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Pin3_INTCFG & (uint32)(~(uint32)position);
    Pin3_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
